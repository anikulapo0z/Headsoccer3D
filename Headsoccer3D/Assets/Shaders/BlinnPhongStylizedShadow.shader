Shader "PrasinShaders/BlinnPhongStylizedShadow"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(Varyings i) : SV_Target
			{
				...
				float3 diffuse =  max(0, dot(normal, mainLight.direction));
				...
				*/
				...
				// Light (total) = L (amb) + L(diffuse) + L(Specular) + L(fresnel)
				//but no fresnel here
				float4 finalLighting = float4(mainLight.color.rgb * diffuse, 1.0f) + float4(_AmbientColor.rgb * ambient, 1.0f) + float4(_SpecularColor.rgb * specular, 1.0f);
				
				//and now we crosshatch the shadows on this
				float shadow = mainLight.shadowAttenuation;
				//shadow = step(0.5, shadow); // hard sharpening threshold for cartoon-like shadows
				...
				return finalLighting * shadowPattern;
			}
